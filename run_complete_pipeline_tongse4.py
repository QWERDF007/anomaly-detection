#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""End-to-End Multi-GPU Pipeline & Experimental Threshold Sweep for 铜色异常检测4相机 Dataset."""
...
outs_dir = Path("/data/wt/report/0827-铜色异常检测4相机")
splits_dir = outs_dir / "data_splits"
bank_data_path = Path("/data/wt/ramdisk/铜色异常检测4相机/建库数据")
test_full_txt = splits_dir / "test_full.txt"

# 1. Parse LabelMe Bank items
...
# 2. Worker process for a dedicated GPU
def _worker_process(gpu_id, task_queue, result_queue):
    ...
    while True:
        ...
        log(f"=== Starting Task: N={n} Size={sz} BatchSize={bs} ===")
        ...
        log(f"DONE N={n} Size={sz} in {elapsed:.1f}s: E2E AUROC={m_e2e["auc"]:.4f}, F1={m_e2e["f1"]:.4f} (opt_th={m_e2e["th"]:.4f}), Din F1={m_din["f1"]:.4f}, Pat F1={m_pat["f1"]:.4f}")
...
def main():
    ...
    for sz in sizes:
        ...
        for n in ns:
            ...
            tasks.append({"n": n, "sz": sz, "bs": bs, "train_txt": train_txt})

    print(f"Launching {len(tasks)} tasks across 8 GPUs for 铜色异常检测4相机...")
