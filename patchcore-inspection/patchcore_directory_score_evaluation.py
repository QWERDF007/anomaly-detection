...
def _evaluate_directory(
    ...
    for image_path in tqdm(
        _iter_images(images_dir), desc=f"Evaluate {data_directory.name}", unit="image", dynamic_ncols=True
    ):
        score_path = _find_score(image_path, data_directory, score_index)
...
def _write_results(
    ...
    result = {str(directory): dict(metrics) for directory, metrics in results.items()}
    ...
    with (output_dir / "metrics.csv").open("w", newline="", encoding="utf-8") as file:
        ...
        writer.writeheader()
        for directory, metrics in result.items():
            writer.writerow({"directory": directory, **{name: metrics[name] for name in METRIC_NAMES}})
    write_per_image_pixel_metrics(
        records,
        output_dir / "pixel_metrics.csv",
        extra_fields=("directory", "mask_path", "image_score_source"),
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    directories = _iter_data_directories(data_root, excluded_directories=[output_dir, *score_dirs])
    score_index = _build_score_index(score_dirs)
    ...
    for data_directory, images_dir, masks_dir in directories:
        ...
        print("  " + "  ".join(f"{name}={metrics[name]:.6f}" for name in METRIC_NAMES), flush=True)
