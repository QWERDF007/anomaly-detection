#!/usr/bin/env python3
"""Run all N=400 experiments (Dinomaly2 & PatchCore across sizes 224, 448, 672) across GPUs."""
...
GPU_LIST = [1, 5, 2, 0, 7, 3, 4, 6]


def generate_n400_tasks() -> List[Dict[str, Any]]:
    tasks = []
    ...
    for size in image_sizes:
        ...
        exp_id = f"dinomaly2_n400_s{size}"
        ...
        tasks.append({
            ...
            "train_file": str(SPLITS_DIR / "train_400.txt"),
            "test_file": str(SPLITS_DIR / "test_400.txt"),
            "out_dir": str(out_dir),
            "cmd": [
                PYTHON_BIN,
                str(BASE_DIR / "Dinomaly2" / "dinomaly_2D.py"),
                "--data_path", str(SPLITS_DIR / "train_400.txt"),
                "--dataset", "custom",
                "--image_size", str(size),
                "--crop_size", str(size),
    ...
    for size in image_sizes:
        batch_size = 16 if size == 224 else (8 if size == 448 else 4)
        exp_id = f"patchcore_n400_s{size}"
        out_dir = OUT_ROOT / exp_id
        tasks.append({
            "exp_id": exp_id,
            "model": "PatchCore",
...
def main():
    print("=" * 80)
    print("Running N=400 Experiments (Dinomaly2 & PatchCore, 6 combinations)")
    print(f"GPUs available: {len(GPU_LIST)} ({GPU_LIST})")
    print("=" * 80)

    tasks = generate_n400_tasks()
    task_q = queue.Queue()
    for t in tasks:
        task_q.put(t)
    ...
    print("=" * 80)
    print(f"ALL {len(tasks)} N=400 EXPERIMENTS COMPLETED in {total_time:.1f}s ({total_time/60:.2f} mins)!")
    print("=" * 80)
