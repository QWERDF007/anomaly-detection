...
def eval_patchcore_variant(
    ...
    print(f"Evaluating PatchCore Model: {pkl_path} on {test_txt.name} ...")
...
def main():
    ...
    parser.add_argument('--subdir', type=str, default='all', choices=['all', 'ori-10', 'ori-21'])
    ...
    all_models = [
        # ori-10
        ("ori-10", "PatchCore (Coreset 10%)", "patchcore", exp_dir / "ori-10_patchcore_n200", exp_dir / "data_splits" / "ori-10_test.txt"),
        ("ori-10", "Dinomaly2 Base (Baseline)", "dinomaly", exp_dir / "ori-10_dinomaly2_base", exp_dir / "data_splits" / "ori-10_test.txt"),
        ("ori-10", "Source-DG (Adapter)", "dinomaly", exp_dir / "ori-10_source_dg_adapter", exp_dir / "data_splits" / "ori-10_test.txt"),
        ("ori-10", "Source-DG (Full + UOT)", "dinomaly", exp_dir / "ori-10_source_dg_full_uot", exp_dir / "data_splits" / "ori-10_test.txt"),
        # ori-21
        ("ori-21", "PatchCore (Coreset 10%)", "patchcore", exp_dir / "ori-21_patchcore_n200", exp_dir / "data_splits" / "ori-21_test.txt"),
        ("ori-21", "Dinomaly2 Base (Baseline)", "dinomaly", exp_dir / "ori-21_dinomaly2_base", exp_dir / "data_splits" / "ori-21_test.txt"),
        ("ori-21", "Source-DG (Adapter)", "dinomaly", exp_dir / "ori-21_source_dg_adapter", exp_dir / "data_splits" / "ori-21_test.txt"),
        ("ori-21", "Source-DG (Full + UOT)", "dinomaly", exp_dir / "ori-21_source_dg_full_uot", exp_dir / "data_splits" / "ori-21_test.txt"),
    ]

    if args.subdir != 'all':
        models_to_eval = [m for m in all_models if m[0] == args.subdir]
    else:
    ...
    out_json = exp_dir / f"benchmark_results_{args.subdir}.json"
