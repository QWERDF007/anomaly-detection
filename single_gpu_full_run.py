...
OUTS = Path(r"F:\tmp\outs")
LOG = OUTS / "full_run.log"
ERR = OUTS / "full_run.err"
SUMMARY = OUTS / "full_run_summary.json"
...
def main():
    ...
    log("=== Single-GPU Full Run Start ===")
    log(f"PYTHON={PYTHON} exists={PYTHON.is_file()}")
    import torch
    log(f"torch {torch.__version__} cuda={torch.cuda.is_available()} device={torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'cpu'}")
    train_sizes = [50,100,200,400]
    image_sizes = [224,448,672]
    batch_map = {224:8, 448:4, 672:2}
    max_iters = 2000
    # Ensure splits exist
    splits = OUTS / "data_splits"
    ...
    for sz in image_sizes:
        ...
        for n in train_sizes:
            ...
            d_save = OUTS / f"dinomaly2_n{n}_s{sz}_seed2024"
            p_save = OUTS / f"patchcore_n{n}_s{sz}_seed2024"
    ...
    log("\n=== Benchmark Speed ===")
    cmd = [str(PYTHON), str(ROOT / "benchmark_speed.py"), "--gpus", "0", "--outs_dir", str(OUTS)]
    ...
    for sz in image_sizes:
        for n in train_sizes:
            ...
            model = sorted(d_models, key=lambda p: p.stat().st_mtime, reverse=True)[0]
            test_txt = splits / f"test_{n}_seed2024.txt"
            out_e2e = OUTS / f"e2e_out_n{n}_s{sz}"
    ...
    cmd = [str(PYTHON), str(ROOT / "generate_final_report_multisize.py")]
    run_cmd(cmd, cwd=str(ROOT))
    # Also run analyze_and_report
    cmd = [str(PYTHON), str(ROOT / "analyze_and_report.py"), "--outs_dir", str(OUTS)]
    run_cmd(cmd, cwd=str(ROOT))
    total = time.perf_counter() - total_t0
    log(f"\n=== ALL DONE in {total:.1f}s ({total/3600:.2f}h) ===")
    SUMMARY.write_text(json.dumps({"finished": datetime.now().isoformat(), "total_sec": total, "results": results}, ensure_ascii=False, indent=2), encoding="utf-8")
    return 0
