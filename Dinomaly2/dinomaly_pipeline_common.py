...
def collect_cached_score_samples(
    groups: Dict[str, object],
    output_dir: Path,
) -> List[Dict]:
    """Build sample metadata from score maps generated by the score pipeline.

    The image roots and group layout are resolved from the dataset, while the
    actual score maps are read from ``output_dir/scores``.  No model or image
    inference is performed here, which makes this helper suitable for a
    standalone evaluation entry point.
    """

    samples: List[Dict] = []
    missing: List[Path] = []
    display_names = dict(GROUPS)
    for group_key, _display_name in GROUPS:
        ...
        for image_root in roots:
            for image_path in iter_image_paths(image_root):
                relative = image_path.relative_to(image_root).with_suffix(
                    ".npy"
                )
                score_path = (
                    artifact_root(
                ...
                samples.append(
                    {
                        "group_key": group_key,
                        "group_label": display_names[group_key],
    ...
    if missing:
        ...
        raise FileNotFoundError(
            f"Missing {len(missing)} cached score map(s) under {output_dir}. "
            "Run dinomaly_score_visualization first.\n"
            f"{preview}{suffix}"
        )
    if not samples:
        raise RuntimeError(
            f"No cached score maps were found under {output_dir}."
        )
    return samples
