...
def main():
    ...
    sub_models = {s: {} for s in subdirs}

    for k, v in data.items():
        ds = v['dataset']
        m_name = v['model']
        if ds in sub_models:
            sub_models[ds][m_name] = v['metrics']
    ...
    # 1. Clean In-Domain Performance
    fig, axes = plt.subplots(1, 2, figsize=(14, 6), sharey=True)
    for ax_idx, s_name in enumerate(subdirs):
        ax = axes[ax_idx]
        ds_dict = sub_models[s_name]
        ...
        ax.set_title(f'Clean In-Domain Metrics: {s_name} (N=200)', fontsize=14, fontweight='bold', pad=12)
        ax.set_xticks(x)
    ...
    for ax_idx, s_name in enumerate(subdirs):
        ...
        ax.set_title(f'False Positive Rate across Distribution Shifts: {s_name}', fontsize=13, fontweight='bold', pad=12)
        ax.set_xticks(x)
        ax.set_xticklabels([m.replace(" ", "\n") for m in model_order], fontsize=10)
        max_val = max(unseen_fpr + seen_fpr + [10])
    ...
    # 3. Fine-grained 9 Shift Breakdown
    shifts_list = [
    ...
    for ax_idx, s_name in enumerate(subdirs):
        ...
        ax.set_title(f'False Positive Rate by Specific Shift Category: {s_name}', fontsize=13, fontweight='bold')
    ...
    fps_list, lat_list, vram_list = [], [], []

    for m in model_order:
        fps_val = np.mean([sub_models[s].get(m, {}).get('efficiency', {}).get('fps', 0) for s in subdirs])
        lat_val = np.mean([sub_models[s].get(m, {}).get('efficiency', {}).get('latency_ms', 0) for s in subdirs])
        vram_val = np.mean([sub_models[s].get(m, {}).get('efficiency', {}).get('vram_gb', 0) for s in subdirs])
    ...
    ax2_twin = ax2.twinx()
    ax2.bar(y_pos - 0.2, lat_list, 0.4, label='Latency (ms)', color='#8338EC', alpha=0.85)
    ax2_twin.bar(y_pos + 0.2, vram_list, 0.4, label='Peak VRAM (GB)', color='#FF006E', alpha=0.85)
